...
When designing an AI based Chat Bot conversation there are a number of pieces that need to be taken into account, these include the following;
 ...
 - **Dialog Flow and Decision Points:** Based on the question and/or response from the user (or the Bot) move the conversation forward to another step in the dislog.
...
Furthermore, the environment provides the ability to test the Natural Language Processing (NLP) and it’s impact on the dialogue flow in real time (through the tester interface), allowing the Bot developer to fine tune and re-train the chatbot as required to improve it’s domain understanding and hence end user experience.
